...
void resolveTransactionCodes(void) {
    ...
    if (resolveAllFields(jar_path, &observer, &rootTask, &stackTask, &fg,
        ...
        if (out_cache_f) {
            ...
            ok = ok && fprintf(out_cache_f, "TRANSACTION_getFocusedStackInfo=%u\n", stackTask) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_onForegroundActivitiesChanged=%u\n", fg) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_isInteractive=%u\n", is_interactive) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_isPowerSaveMode=%u\n", is_power_save) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_getBrightness=%u\n", get_brightness) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_onDisplayEvent=%u\n", on_display_event) > 0;
            ok = ok && fprintf(out_cache_f, "TRANSACTION_registerCallbackWithEventMask=%u\n", register_cb_mask) > 0;
